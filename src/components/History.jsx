        ...
        <div className="text-center mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-16">
            {data.achievements.map((achievement, index) => (
              <div
                key={index}
                className="flex flex-col items-center gap-3 p-5 rounded-2xl bg-white border border-slate-100 shadow-md text-slate-800 font-bold transition-all hover:bg-emerald-600 hover:text-white hover:shadow-xl hover:translate-y-[-3px] duration-300"
              >
              ...
              return (
                <div key={index} className="relative group">
                  <div className="flex gap-5 md:gap-8">
                    {/* Serial Number Circle — desktop only (left timeline) */}
                    <div className="relative z-10 shrink-0 hidden md:flex">
                      <div
                        className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br ${gradient} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300`}
                      >
                    ...
                    {/* Content Card */}
                    <div className="flex-1 relative p-6 md:p-8 rounded-2xl bg-white border border-slate-100 shadow-md hover:shadow-xl hover:translate-y-[-2px] transition-all duration-400">
                      {/* Serial badge — mobile only (top-right corner) */}
                      <div
                        className={`absolute -top-3 -left-3 md:hidden w-10 h-10 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center shadow-lg z-20`}
                      >
                      ...
                      {section.tags && (
                        <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-slate-100">
                          <span className="text-sm text-slate-400 font-semibold mr-1">
                            পাড়াসমূহ:
                          </span>
                          {section.tags.map((tag, i) => (
                            <span
                              key={i}
                              className="px-3 py-1 rounded-full bg-violet-50 text-violet-700 text-sm font-semibold border border-violet-100 hover:bg-violet-100 transition-colors"
                            >
                              {tag}
                            </span>
                          ))}
                      ...
                      {section.crops && (
                        <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-slate-100">
                          ...
                          {section.crops.map((crop, i) => (
                            <span
                              key={i}
                              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-sm font-semibold border border-emerald-100"
                            >
                              <Wheat size={13} />
                              {crop}
                            </span>
