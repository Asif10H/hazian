...
const Representatives = ({ data }) => {
  return (
    <section id="representatives" className="section-padding bg-slate-50 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-20">
           <h2 className="text-4xl md:text-6xl font-bold font-sans text-slate-900 mb-6">{data.title}</h2>
           <p className="text-xl text-slate-500 max-w-2xl mx-auto">আমাদের গ্রামের উন্নয়নে নিয়োজিত জনপ্রতিনিধি এবং সমাজ সংস্কারকবৃন্দ।</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
           {data.list.map((rep, index) => (
             <div key={index} className="relative p-10 bg-white border border-slate-100 rounded-[3rem] shadow-xl hover:shadow-emerald-500/10 transition-all duration-500 group text-center hover:scale-[1.02]">
                ...
                <h3 className="text-3xl font-black font-sans text-slate-900 mb-2 truncate">{rep.name}</h3>
